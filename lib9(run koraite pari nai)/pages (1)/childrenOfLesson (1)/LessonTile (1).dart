...
class _LessonTileState extends State<LessonTile> {
  ...
  Future<void> loadData() async {
    try {
      ...
      setState(() {
        modules = moduleList.map((module) => module as Map<String, dynamic>).toList();
      });
    } catch (e) {
      print("Error loading lesson data: $e");
    }
  ...
  void navigateToHomePage(BuildContext context, String moduleName, String subModuleName) {
    ...
    //   ),
    // );
    print('Navigate to HomePage with module: $moduleName, subModule: $subModuleName');
  }
        ...
        child: modules.isEmpty
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, moduleIndex) {
                  ...
                  final List<dynamic> subModules = module['subModules'] as List<dynamic>;
                  return ExpansionTile(
                    title: Text(module['name'] as String, style: const TextStyle(fontWeight: FontWeight.bold)),
                    children: subModules.map((subModule) {
                      final List<dynamic> lessons = subModule['lessons'] as List<dynamic>;
                      return GestureDetector(
                        onTap: () {
                          navigateToHomePage(context, module['name'] as String, subModule['name'] as String); // Navigate to HomePage on tap
                        },
                        child: ExpansionTile(
                          title: Text(subModule['name'] as String, style: const TextStyle(fontWeight: FontWeight.w600)),
                          children: [
                            FutureBuilder<List<String>>(
                              future: fetchImageUrls(module['name'] as String, subModule['name'] as String),
                            ...
                            ...lessons.map((lesson) {
                              return ListTile(
                                title: Text(lesson['name'] as String), // Assuming lesson structure has 'name'
                              );
                            }).toList(),
                          ],
                        ),
